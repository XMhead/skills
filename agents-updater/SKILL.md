...
- **agents-report**：审计或治理 `AGENTS.md` 时使用。先产出证据报告和取舍项，未经用户明确选择不自动删除、合并或改写规则。
...
7. 扫描历史记录时，只统计真实用户消息；排除子 Agent 任务单、工具调用、shell、系统/开发者/环境上下文、浏览器上下文和 trace 重复。
8. 检查上下文浪费信号：如果以后仍会重复消耗上下文，补索引、补入口、拆 ref 或加脚本。
9. 验证：运行可用的静态检查、脚本 `--help` 或轻量样例；对于 hooks/commands/tools，检查配置文件语法和 CLI 是否识别；至少 grep 旧名和新名确认引用收敛。
...
- 同时出现两个以上信号时触发 `pollution-breaker`：停止扩展搜索，不删除或合并文件，先生成污染报告。
- 报告至少包含：原任务、已读取来源、直接相关来源、疑似无关/重复内容、冲突、对路线和成本的影响、建议动作。
- 报告末尾只提出一个取舍问题，例如继续最小流程、暂停治理污染，或忽略指定的非相关来源；不得用泛泛的“为什么有这么多信息”代替选择题。
- 污染不阻塞当前目标时标记 `deferred-cleanup`，完成最小交付后再记录治理候选；不得把治理任务混入当前执行流。
- 只有用户选择治理或污染已直接阻塞交付时，才进入第二条治理流；任何清理仍按 `agents-report` 规则先举证。
...
- 默认把 `AGENTS.md` 当作项目规则报告对象：先记录路径、行号、规则类别、证据来源、消费者、冲突和建议，不因降噪自动删除或合并。
...
- 用户明确批准后再做最小范围补丁，并复核无关工作区修改、规则行号和引用收敛。
...
3. 本轮是否有真实踩坑，且不能靠删除旧信息避免？有才按现象、根因、正确做法三段式写入。
4. 本轮是否涉及 2 个以上插件或系统？有就核对项目 linkage 是否对齐。
5. 本轮是否暴露项目指令文件缺少硬规则？有就写项目私有规则。
6. 本轮是否反复消耗上下文做同类搜索、阅读或临时脚本？有就补索引、拆 ref 或沉淀脚本。
